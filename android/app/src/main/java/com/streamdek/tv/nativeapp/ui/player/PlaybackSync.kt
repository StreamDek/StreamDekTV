 ...
 *
 * A property of what the sound comes out of - a soundbar, an AV receiver or Bluetooth headphones
 * that lag - rather than of any one video, so it is kept on the device and never synced: the phone
 * in the next room has different speakers. Adjusting the delay in the player changes only the
 * video playing; this is the starting point each new one gets. Kept at zero by default, so nothing changes for
 * anyone who never opens it.
 */
internal object AudioSyncOptions {
  private const val PREFS_NAME = "streamdek_tv_audio_sync"
