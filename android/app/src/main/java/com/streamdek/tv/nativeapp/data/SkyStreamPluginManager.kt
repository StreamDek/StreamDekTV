...
/**
 * Runs SkyStream provider collections (`.sky` extensions) on the television.
 *
 * Ported from the phone's manager. The television does not add or change collections through this
 * class: they arrive in the `skystream` section of the profile's synced plugin document
 * ([restoreCloudState]) — added on the phone or the web portal — and the settings screen writes
 * its own changes back through that document, which then arrives here the same way.
 *
...
class SkyStreamPluginManager(private val context: Context) {
  ...
  /**
   * Fetches any switched-on source whose bundle is missing from disk — cleared app data, or a new
   * version arriving from the account — then probes what they offer.
   */
