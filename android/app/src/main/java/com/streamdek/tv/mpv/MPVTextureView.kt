...
class MPVTextureView @JvmOverloads constructor(
    ...
    defStyleAttr: Int = 0,
) : TextureView(context, attrs, defStyleAttr), TextureView.SurfaceTextureListener, MPVLib.EventObserver, MPVLib.LogObserver, MpvPlayerController {
    ...
    init {
        ...
        isOpaque = false
        keepScreenOn = true
        // The video surface must never take D-pad focus away from the Compose
        // controls. When it did, left/right presses aimed at the progress bar were
        // swallowed by the surface, which made scrubbing require repeated presses.
        // Remote key handling lives in the Compose layer instead.
        isFocusable = false
    ...
    override fun setDecoderMode(mode: String?) {
        preferredDecoderMode = when (mode?.trim()?.lowercase()) {
            "hardware", "hw", "mediacodec" -> "hardware"
